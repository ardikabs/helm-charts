...
|-----|------|---------|-------------|
| affinity | string | `nil` | specifies pod affinity rules |
...
| app.volumeMounts | list | `[]` | specifies mount point from list of extra volumes attached to the application |
| env | list | `[]` | pod customized environment variable injected to the application container |
| envFrom | list | `[]` | pod customized environment variable from selected reference to be injected to the application container |
| extraVolumes | list | `[]` | specifies a list of extra volumes to be attached to the pod |
...
| job.volumeMounts | list | `[]` | specifies mount point from list of extra volumes attached to the application |
| nameOverride | string | `""` | defines as a release-related resource name |
| nodeSelector | object | `{}` | describes which node must be used for the pod |
| ownerReference.domain | string | `"common"` | specifies the application belong to which product domain |
| ownerReference.team | string | `"core"` | specifies the application belong to which team |
| podAnnotations | object | `{}` | additional annotations attached to the pods |
| podLabels | object | `{}` | additional labels attached to the pods |
| podLastRestart | int | `0` | the value that intend for restarting the pod |
| securityContext | object | `{}` | specifies security context applied to the pod |
| terminationGracePeriodSeconds | int | `30` | duration in seconds the pod needs to terminate gracefully |
| tolerations | list | `[]` | describes which toleration used for the pod to specify target node |
